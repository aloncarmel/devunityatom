...
firepad.AtomAdapter = AtomAdapter = (function() {
  ...
  function AtomAdapter(atom) {
    ...
    this.buffer = atom.getBuffer();
    this.grabDocumentState();

    //Fixing new API

    this.disposechange = this.buffer.onDidChange(this.onChange);

    this.onCursorActivity = __bind(this.onCursorActivity, this);

    //Fixing new API
    this.buffer.onDidUpdateMarkers(this.onCursorActivity);

    this.atom.onDidChangeSelectionRange(this.onCursorActivity);
    //this.atom.onDidRemoveSelection(this.changeSelection);
    //this.atom.onDidAddSelection(this.changeSelection);
  ;
  }
  AtomAdapter.prototype.changeSelection = function(event) {

    console.log(event)

  }
  AtomAdapter.prototype.removeSelection = function(selection) {
  ...
  AtomAdapter.prototype.posFromIndex = function(index) {
    var line, row, _i, _len, _ref;

    _ref = this.buffer.getLines();
  ...
  AtomAdapter.prototype.trigger = function() {
    var args, event, _ref, _ref1;
    event = arguments[0], args = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
    return (_ref = this.callbacks) != null ? (_ref1 = _ref[event]) != null ? _ref1.apply(this, args) : void 0 : void 0;
  };
  AtomAdapter.prototype.addStyleRule = function(css) {
    ...
    if (!this.addedStyleRules) {
      ...
      this.addedStyleRules = {};
      styleElement = document.createElement('style');
      styleElement.id = 'firepad-user-colors';
      styleElement.context = 'atom-text-editor';
      document.documentElement.getElementsByTagName('head')[0].appendChild(styleElement);
      this.addedStyleSheet = styleElement.sheet;
    ...
    return document.getElementById("firepad-user-colors").appendChild(textnode);
  };
  AtomAdapter.prototype.indexFromPos = function(pos, lines) {
    var i, index, _i, _ref;
    if (lines == null) {
      lines = this.lastDocLines;
    }
  ...
  AtomAdapter.prototype.detach = function() {
    ...
    return this.disposechange.dispose();
  };
  ...
  AtomAdapter.prototype.setOtherCursor = function(cursor, color, clientId) {
    ...
    if (cursorRange) {
      ...
      cursorRange.selection.cursor.destroy();
      cursorRange.decoration.destroy();
      cursorRange.selection.marker.destroy();
    ...
    css = ".atom-text-editor ."+clazz+" .region , atom-text-editor::shadow ."+clazz+"  .region  {\n  position: absolute;\n z-index:0; background: " + (justCursor ? 'transparent' : color) + ";opacity:0.5;}";
    ...
    //cursorRange.addedcursor = this.atom.addCursorAtBufferPosition([start.row,start.column]);
    cursorRange.decoration = this.atom.decorateMarker(cursorRange.id, {type:'highlight',class:clazz});
